...
(function () {
  ...
  function injectChrome() {
    ...
    document.body.insertAdjacentHTML('afterbegin', `
      ...
      <aside class="drawer" id="drawer">
        ...
        <div class="df">
          ...
          <a href="/store#checkout" class="btn fill block" id="checkoutBtn" data-track="cart:checkout">PROCEED TO CHECKOUT <span class="arr"></span></a>
        </div>
      </aside>
    ...
    if (!$('.footer')) document.body.insertAdjacentHTML('beforeend', `
      <footer class="footer">
        <div class="f-grid">
          <div><h4>Layer Zero · 3D Print Lab</h4><p id="fAddr">5400 W Atlantic Blvd, Margate, FL 33063</p><p id="fHours">Mon–Sat 09:00–19:00</p><p class="mono" style="margin-top:14px">26.2445° N / 80.2064° W</p></div>
          <div><h4>Navigate</h4>${NAV.map(([h, t]) => `<a href="${h}">${t}</a>`).join('')}<a href="/store#track">Track order</a></div>
          <div><h4>Services</h4><a href="/quote">Custom prints</a><a href="/quote">Design & CAD</a><a href="/quote">Batch production</a><a href="/showcase">Case studies</a></div>
          <div><h4>Contact</h4><a id="fMail" href="mailto:orders@layerzero.print">orders@layerzero.print</a><a id="fPhone" href="tel:+19545550142">(954) 555-0142</a><p class="muted" style="font-size:12px;margin-top:14px">Local pickup & courier in Broward · Shipping across Florida and the US.</p></div>
        </div>
        <div class="big">LAYER ZERO</div>
        <div class="bottom"><span>© ${new Date().getFullYear()} LAYER ZERO 3D PRINT LAB</span><span>MARGATE · FLORIDA · USA</span><span><a href="/master" style="display:inline;margin:0;font-size:inherit;color:inherit;opacity:.6">MASTER</a></span></div>
      </footer>`);
  ...
  function transitions() {
    ...
    document.addEventListener('click', e => {
      ...
      if ((pathPart || PAGE) === PAGE || pathPart === PAGE + '.html') return; // same page anchors handled natively
      if (REDUCED) return;
  ...
  const cart = {
    ...
    render() {
      ...
      const box = $('#cartItems');
      box.innerHTML = this.items.length ? this.items.map(i => `<div class="ci"><img src="${esc(i.image)}" alt=""><div><div class="n">${esc(i.name)}</div>${i.variant ? `<div class="v">${esc(i.variant)}</div>` : ''}<div class="q"><button data-k="${esc(i.key)}" data-d="-1">−</button><span>${i.qty}</span><button data-k="${esc(i.key)}" data-d="1">+</button></div></div><div><div class="p">${fmt(i.price * i.qty)}</div><button class="rm" data-k="${esc(i.key)}" data-d="rm">remove</button></div></div>`).join('') : '<div class="empty-note">Cart is empty<br><br><a href="/store" class="link-arrow">Visit the store</a></div>';
    ...
    init() {
      ...
      $('#checkoutBtn').addEventListener('click', () => { this.close(); if (PAGE === '/store') { const c = $('#checkout'); if (c) c.scrollIntoView({ behavior: 'smooth' }); } });
